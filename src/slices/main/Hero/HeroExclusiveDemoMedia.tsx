...
/**
 * The interactive demo is desktop-only. Below 1080px wide (tablet and smaller)
 * we render the static hero image instead, and the demo iframe is never mounted
 * so those devices don't pay for it. Gating is width-only so short laptops
 * (e.g. 1280x800) still get the demo.
 */
const DEMO_MEDIA_QUERY = '(min-width: 1080px)'

type HeroExclusiveDemoMediaProps = {
  fullBleedImage: Content.HeroSliceExclusiveDemoPrimary['full_bleed_image']
  imageMobile: Content.HeroSliceExclusiveDemoPrimary['image_mobile']
  isShortImage: Content.HeroSliceExclusiveDemoPrimary['is_short_image']
}

export default function HeroExclusiveDemoMedia({
  fullBleedImage,
  imageMobile,
  isShortImage,
}: HeroExclusiveDemoMediaProps) {
